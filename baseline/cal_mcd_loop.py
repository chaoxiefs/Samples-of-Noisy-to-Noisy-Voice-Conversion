...
#########################################################################
VC_DATASET_DIR = Path("convert")

REF_DIR = Path("*/esc50-all/norm_vcc2018_eval_16k_snr5")

MCD_SAVE_DIR = Path("MCD") / VC_DATASET_DIR
...
for step in Path(VC_DATASET_DIR).iterdir():
...
for step in steps:
    ...
    for t_spk in TARGET_SPEAKERS:
        ...
        for vc_spk in VC_SPEAKERS:
            vc_wav_list = list((VC_DATASET_DIR / step /"T_{}".format(t_spk) / vc_spk).glob("*.wav"))
