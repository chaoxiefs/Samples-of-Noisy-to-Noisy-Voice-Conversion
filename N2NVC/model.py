""" Based on https://github.com/bshall/ZeroSpeech/blob/master/model.py """
...
class Encoder(nn.Module):
    ...
    def encode(self, mel):
        mel = self.pad_tensor(mel.transpose(1, 2), pad=1, side='both')
        z = self.encoder(mel.transpose(1, 2))
        z, indices = self.codebook.encode(z.transpose(1, 2))
        return z, indices

    def set_codebook(self, codebook):
        self.codebook.set_codebook(codebook)
...
class VQEmbeddingEMA(nn.Module):
    ...
    def set_codebook(self, codebook):
        self.register_buffer("embedding", codebook)
        self.embedding = codebook


class Decoder(nn.Module):
    def __init__(self, in_channels, n_speakers, speaker_embedding_dim,
                 conditioning_channels, mu_embedding_dim, n_embedding_dim, rnn_channels,
        ...
        self.n_embedding = nn.Embedding(self.quantization_channels, n_embedding_dim)
        self.rnn2 = nn.GRU(mu_embedding_dim + 2*conditioning_channels + n_embedding_dim, rnn_channels, batch_first=True)
        self.fc1 = nn.Linear(rnn_channels, fc_channels)
        self.fc2 = nn.Linear(fc_channels, self.quantization_channels)

    def get_spk_embed(self, speakers):
        return self.speaker_embedding(speakers)


    def forward(self, x, n, z, speakers):
        #z = F.interpolate(z.transpose(1, 2), scale_factor=2)
        #z = z.transpose(1, 2)

        speakers = self.speaker_embedding(speakers)
        #speakers = self.speaker_linears(speakers)
        ...
        n = self.n_embedding(n)
        x, _ = self.rnn2(torch.cat((x, z, n), dim=2))
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x

    def generate(self, z, n, speaker):
        output = []
        ...
        #speaker = self.speaker_linears(speaker)
