...
def train_model(conf):
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        for i, (noise, label, mels, speakers) in enumerate(tqdm(train_loader), 1):
            noise, label, mels, speakers = noise.to(device), label.to(device), mels.to(device), speakers.to(device)


            optimizer.zero_grad()
            with amp.autocast(enabled=True):
                z, vq_loss, perplexity = encoder(mels)
                output = decoder(label[:, :-1], noise[:, :], z, speakers)
                recon_loss = F.cross_entropy(output.transpose(1, 2), label[:, 1:])
        ...
        print("epoch:{}, steps:{}, recon loss:{:.2E}, vq_loss:{:.2E}, perpexlity:{:.3f}"
              .format(epoch, global_step, average_recon_loss, average_vq_loss, average_perplexity))

        encoder.eval()

        with torch.no_grad():
            for i, (noise, label, mels, speakers) in enumerate(tqdm(eval_loader), 1):
                noise, label, mels, speakers = noise.to(device), label.to(device), mels.to(device), speakers.to(device)
                z, eval_vq_loss, eval_perplexity = encoder(mels)
                output = decoder(label[:, :-1], noise[:, :], z, speakers)
                eval_recon_loss = F.cross_entropy(output.transpose(1, 2), label[:, 1:])
            ...
            print("epoch:{}, steps:{}, EVAL recon loss:{:.2E}, EVAL vq_loss:{:.2E}, EVAL perpexlity:{:.3f}"
